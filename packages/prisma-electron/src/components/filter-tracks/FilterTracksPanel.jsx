...
class FilterTracksPanel extends React.Component {
  ...
  drawImage = (imgList, ctx, icon, isCustom) => {
    ...
    img.onload = () => {
      ...
      let { displayWitdh, displayHeight } = this.scaleImage(imgWidth, imgHeight);
      ...
      x -= displayWitdh;
      ctx.drawImage(img, 0, 0, imgWidth, imgHeight, x, y, displayWitdh, displayHeight);

      if (!isCustom) {
        ctx.globalCompositeOperation = "source-atop";
        ctx.globalAlpha = icon.opacity;
        ctx.fillStyle = icon.color;
        ctx.fillRect(x, y, displayWitdh, displayHeight);
        ctx.globalCompositeOperation = 'source-over';
        ctx.globalAlpha = 1;
      }

      img.width = displayWitdh;
